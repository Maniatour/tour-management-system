-- reservation_choices 테이블의 중복 데이터를 option_name으로 정리 (UUID 호환 버전)
...
SELECT 
  ...
  co.option_name_ko,
  co.option_name,
  MIN(rc.choice_id::text)::uuid as choice_id,  -- 문자열로 변환 후 MIN 적용
  MIN(rc.option_id::text)::uuid as option_id,  -- 문자열로 변환 후 MIN 적용
  SUM(rc.quantity) as total_quantity,
