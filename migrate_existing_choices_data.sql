-- 기존 JSONB choices 데이터를 새로운 정규화된 구조로 마이그레이션
-- 기존 데이터를 보존하면서 새로운 구조로 변환
...
-- 2. 기존 예약 choices 데이터 백업
CREATE TABLE IF NOT EXISTS reservations_choices_backup AS
SELECT id, choices, created_at
FROM reservations
WHERE choices IS NOT NULL;

-- 3. 기존 JSONB choices를 새로운 구조로 변환하는 함수
CREATE OR REPLACE FUNCTION migrate_existing_choices()
RETURNS TABLE(
  product_id TEXT,
    ...
    IF product_record.choices ? 'required' AND jsonb_typeof(product_record.choices->'required') = 'array' THEN
      ...
      LOOP
        ...
        INSERT INTO product_choices (
          ...
          product_record.id,
          COALESCE(choice_item->>'id', 'default_choice'),
        ...
        IF choice_id IS NULL THEN
          SELECT id INTO choice_id
          FROM product_choices
          WHERE product_id = product_record.id 
          AND choice_group = COALESCE(choice_item->>'id', 'default_choice');
        END IF;
...
-- 4. 기존 예약 choices를 새로운 구조로 변환하는 함수
CREATE OR REPLACE FUNCTION migrate_existing_reservation_choices()
RETURNS TABLE(
  reservation_id TEXT,
  migrated_choices INTEGER
) AS $$
  ...
  LOOP
    ...
    IF reservation_record.choices ? 'required' AND jsonb_typeof(reservation_record.choices->'required') = 'array' THEN
      ...
      LOOP
        ...
        IF choice_id IS NOT NULL THEN
          -- 단일 선택인 경우
          IF choice_item ? 'selected' THEN
            SELECT co.id INTO option_id
            FROM choice_options co
            WHERE co.choice_id = choice_id
...
BEGIN
  ...
  -- 상품 choices 마이그레이션
  FOR result IN SELECT * FROM migrate_existing_choices() LOOP
  ...
  -- 예약 choices 마이그레이션
  FOR result IN SELECT * FROM migrate_existing_reservation_choices() LOOP
