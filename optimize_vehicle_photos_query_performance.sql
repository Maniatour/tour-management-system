...
-- vehicle_id로 필터링하고 display_order로 정렬하는 쿼리를 위한 인덱스
-- 주의: INCLUDE 절은 인덱스 행 크기 제한(8KB)을 초과할 수 있으므로 제거
-- photo_url이 매우 길 수 있어서 인덱스 크기 제한 초과 가능
...
-- 주의: INCLUDE 절은 인덱스 행 크기 제한(8KB)을 초과할 수 있으므로 제거
-- photo_url과 description이 매우 길 수 있어서 인덱스 크기 제한 초과 가능
...
CREATE INDEX idx_vehicle_photos_primary_only 
ON vehicle_photos(vehicle_id) 
WHERE is_primary = true;

DROP INDEX IF EXISTS idx_vehicle_type_photos_primary_only;
CREATE INDEX idx_vehicle_type_photos_primary_only 
...
-- ============================================
-- 6. 테이블 최적화 (VACUUM) - 별도 실행 필요
-- ============================================

-- 주의: VACUUM은 트랜잭션 블록 밖에서 실행되어야 합니다
-- 아래 명령어들을 이 스크립트 실행 후 별도로 실행하세요:
...
-- FROM vehicle_photos
-- WHERE vehicle_id IN ('id1', 'id2', 'id3', ...) -- 실제 ID 목록
