...
export default function ChannelOtaReconciliationModal({
  ...
  const [markAuditedOnApply, setMarkAuditedOnApply] = useState(true)
  ...
  const applyOtaAmount = async (row: ReconcileResultRow) => {
    ...
    try {
      ...
      if (markAuditedOnApply && canAudit) {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <label className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer select-none">
              <input
                type="checkbox"
                checked={markAuditedOnApply}
                onChange={(e) => setMarkAuditedOnApply(e.target.checked)}
                disabled={!canAudit}
              />
              반영 시 Audit 완료 표시{!canAudit ? ' (권한 없음)' : ''}
            </label>
          </div>
            ...
            <>
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <button
                  ...
                  className="ml-auto px-3 py-1.5 rounded-md text-xs font-medium border border-amber-300 text-amber-900 bg-amber-50 hover:bg-amber-100 disabled:opacity-50"
                >
