...
export default function PaymentMethodManager({ 
  ...
  const loadMethods = async () => {
    try {
      ...
      const response = await fetchApiWithAuth(`/api/payment-methods?${params}`)
    ...
    try {
      ...
      if (formData.user_emails.length > 0) {
        ...
        while (retryCount < maxRetries) {
          const response = await fetchApiWithAuth('/api/payment-methods', {
            method: 'POST',
            headers: {
        ...
        if (!result.success && result.message?.includes('already exists')) {
          ...
          if (inputId) {
            try {
              const existingResponse = await fetchApiWithAuth(`/api/payment-methods/${id}`)
  ...
  const handleUpdateMethod = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetchApiWithAuth(`/api/payment-methods/${encodeURIComponent(editingMethod.id)}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleDeleteMethod = async (id: string) => {
    ...
    try {
      const response = await fetchApiWithAuth(`/api/payment-methods/${encodeURIComponent(normalizedId)}`, {
        method: 'DELETE'
      })
  ...
  const handleUpdateStatus = async (method: PaymentMethod, newStatus: PaymentMethodStatusValue) => {
    ...
    try {
      const response = await fetchApiWithAuth(`/api/payment-methods/${encodeURIComponent(method.id)}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleBulkEditSubmit = async () => {
    try {
      ...
      for (let i = 0; i < bulkEditRows.length; i++) {
        ...
        try {
          const response = await fetchApiWithAuth(`/api/payment-methods/${encodeURIComponent(id)}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
            },
