...
type WhyChooseLibraryPickerProps = {
  editLocale: AdminEditLocale
  libraryItems: WhyChooseLibraryItem[]
...
export default function WhyChooseLibraryPicker({
  editLocale,
  libraryItems,
  libraryLoading,
  librarySearch,
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div className="text-sm font-medium text-foreground">
                        {item.name ||
                          getWhyChooseLocalizedText(item, 'title', editLocale) ||
                          t('libraryUnnamed')}
                      </div>
                      <ContentLibraryLocaleBadges locales={getWhyChooseFilledLocales(item)} />
                    </div>
                    <div className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">
                      {getWhyChooseLocalizedText(item, 'title', editLocale) || item.title}
                    </div>
                  </div>
                </label>
              ))}
