...
import TourAudienceLibraryManagerModal from '@/components/product/TourAudienceLibraryManagerModal'
...
type TourAudienceItem = TourAudienceLibraryItem & {
  link_id?: string
  product_id: string
  order_index: number
}

type CustomerPageTourAudienceEmbedProps = {
  productId: string
  locale?: string
  onSaved?: () => void
...
function itemLabel(item: TourAudienceItem, locale: AdminEditLocale, empty: string) {
  return getTourAudienceLocalizedText(item, locale).trim() || item.name.trim() || empty
}

function snapshot(items: TourAudienceItem[]) {
  return JSON.stringify(items.map((i) => ({ id: i.id, link_id: i.link_id ?? null, order_index: i.order_index })))
}

export default function CustomerPageTourAudienceEmbed({
  productId,
  locale: localeProp,
  onSaved,
  onDirtyChange,
}: CustomerPageTourAudienceEmbedProps) {
  ...
  const [items, setItems] = useState<TourAudienceItem[]>([])
  const [activeItemId, setActiveItemId] = useState<string | null>(null)
  const [initialSnapshot, setInitialSnapshot] = useState<string | null>(null)
  const [showLibraryPicker, setShowLibraryPicker] = useState(false)
  const [libraryItems, setLibraryItems] = useState<TourAudienceLibraryItem[]>([])
  ...
  const { recommended, notRecommended } = useMemo(() => splitTourAudienceByKind(items), [items])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const nextItems: TourAudienceItem[] = attached.map(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('추천 대상 저장 오류:', error)
  ...
  const openLibraryPicker = async () => {
    ...
    try {
      const rows = await fetchTourAudienceLibrary(supabase as never, {
        activeOnly: true,
      ...
      console.error('추천 대상 library load error:', error)
  ...
  const attachSelected = () => {
    ...
    const newAttached: TourAudienceItem[] = selectedRows.map((row, idx) => ({
      ...row,
      product_id: productId,
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      console.error('추천 대상 삭제 오류:', error)
  ...
  const moveItem = async (direction: 'up' | 'down') => {
    ...
    try {
      ...
      console.error('추천 대상 순서 변경 오류:', error)
  ...
  const renderItemGroup = (groupItems: TourAudienceItem[], kindLabel: string) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {showLibraryManager ? (
        <TourAudienceLibraryManagerModal
          onClose={() => {
