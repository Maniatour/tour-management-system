...
export type TourHotelBookingPendingDeletionRow = {
  id: string;
  tour_id: string | null;
  hotel: string | null;
  city: string | null;
...
type TourHotelBookingDeletionReviewModalProps = {
  open: boolean;
...
export default function TourHotelBookingDeletionReviewModal({
  open,
  onOpenChange,
  onAfterBulkDelete,
  ...
  dialogTitle,
  dialogDescription,
}: TourHotelBookingDeletionReviewModalProps) {
  const locale = useLocale();
  const [rows, setRows] = useState<TourHotelBookingPendingDeletionRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          'id, tour_id, hotel, city, reservation_name, check_in_date, check_out_date, deletion_requested_at, deletion_requested_by, tours(tour_date)'
        )
        .not('deletion_requested_at', 'is', null)
        .order('deletion_requested_at', { ascending: false });
      if (error) throw error;
      setRows((data as TourHotelBookingPendingDeletionRow[]) || []);
      setSelected(new Set());
    } catch (e) {
      console.error(e);
      setRows([]);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const ids = [...selected];
      const { error } = await supabase.from('tour_hotel_bookings').delete().in('id', ids);
      if (error) throw error;
      onAfterBulkDelete?.();
      await load();
    } catch (e) {
  ...
  const defaultTitle =
    locale === 'ko' ? '삭제 요청된 투어 호텔 부킹' : 'Tour hotel bookings pending deletion';
  const title = dialogTitle ?? defaultTitle;
  const defaultDesc = allowBulkPermanentDelete
    ? locale === 'ko'
      ? '관리 직책이 삭제 요청한 항목입니다. SUPER가 확인 후 영구 삭제할 수 있습니다.'
      : 'Soft-deleted by staff. Super admins can permanently delete selected rows.'
    : locale === 'ko'
      ? '목록에서 숨겨진 부킹입니다. 영구 삭제는 SUPER만 할 수 있습니다.'
      : 'Hidden from the main list. Only a super admin can permanently delete.';
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-gray-200 text-left text-xs text-gray-500">
                  ...
                  <th className="py-2 px-1 font-medium">{locale === 'ko' ? '호텔' : 'Hotel'}</th>
                  <th className="py-2 px-1 font-medium">{locale === 'ko' ? '도시' : 'City'}</th>
                  <th className="py-2 px-1 font-medium">{locale === 'ko' ? '예약명' : 'Guest'}</th>
                  <th className="py-2 px-1 font-medium">{locale === 'ko' ? '체크인' : 'Check-in'}</th>
                  <th className="py-2 px-1 font-medium">{locale === 'ko' ? '체크아웃' : 'Check-out'}</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50/80">
                    ...
                    <td className="py-2 px-1 align-middle max-w-[10rem] truncate" title={r.hotel ?? ''}>
                      {r.hotel ?? '—'}
                    </td>
                    <td className="py-2 px-1 align-middle">{r.city ?? '—'}</td>
                    <td className="py-2 px-1 align-middle max-w-[8rem] truncate" title={r.reservation_name ?? ''}>
                      {r.reservation_name ?? '—'}
                    </td>
                    <td className="py-2 px-1 align-middle tabular-nums">{r.check_in_date ?? '—'}</td>
                    <td className="py-2 px-1 align-middle tabular-nums">{r.check_out_date ?? '—'}</td>
