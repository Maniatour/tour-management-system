...
export default function SopSignClient() {
  ...
  const load = useCallback(async () => {
    ...
    const versionQuery = versionParam
      ? supabase
    ...
    const row = data as SopVersion
    setVersion(row)
    const parsed = parseSopDocumentJson(row.body_structure)
    setStructureDoc(parsed)
  }, [authUser?.id, isInitialized, isEn, loading, staffOk, versionParam])
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { error: insErr } = await supabase.from('sop_signatures').insert({
  ...
  if (loadError || !version) {
    return (
      <div className="mx-auto max-w-3xl p-6">
        <p className="text-red-600">{loadError || (isEn ? 'No SOP found.' : 'SOP가 없습니다.')}</p>
      </div>
    )
  }
    ...
    return (
      <div className="mx-auto max-w-3xl p-6 space-y-4">
        ...
        <p className="text-gray-700">
          {isEn
            ? 'This version is already on file.'
            : '이 버전에 대한 서명이 이미 등록되어 있습니다.'}
        </p>
        <Button
          type="button"
          onClick={() =>
