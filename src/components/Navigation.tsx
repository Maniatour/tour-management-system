...
const NavigationContent = () => {
  ...
  const tBrand = useTranslations('customerSiteFooter')
  ...
  return (
    <nav className={`${navShellClass} relative z-50 min-h-[var(--header-height)] flex flex-col text-foreground overflow-visible md:flex-row md:items-center md:h-[var(--header-height)]`}>
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:items-center md:min-h-[var(--header-height)]">
        <div className="flex justify-between items-center w-full min-h-[var(--header-height)] flex-shrink-0">
          {/* 로고 및 제목 */}
          <div className="flex items-center space-x-4">
            <Link href={`/${locale}`} className="flex items-center space-x-2">
              <span className={`font-bold tracking-tight ${isCustomerSite ? 'text-lg text-primary md:text-xl' : 'text-lg md:text-xl text-gray-800'}`}>
                {isCustomerSite ? tBrand('brandName') : t('systemTitle')}
              </span>
            </Link>
            {isSimulating && simulatedUser && (
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <Link href={`/${locale}/reservation-check`} className={linkClass}>
              <Search className="w-4 h-4 mr-2" />
              {t('reservationCheck')}
            </Link>
            {isCustomerSite && (
              <Button variant="booking" size="sm" asChild>
                <Link href={`/${locale}/products`}>{t('browseTours')}</Link>
              </Button>
            )}
            <LanguageSwitcher />
            ...
            <div className="flex flex-col space-y-4 text-gray-900 max-w-7xl mx-auto" style={{ backgroundColor: '#ffffff', color: '#111827' }}>
              ...
              {isCustomerSite && (
                <div className="px-2 pt-2">
                  <Button variant="booking" size="booking" className="w-full" asChild>
