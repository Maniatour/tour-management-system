...
async function callerMayInsertCustomers(
  ...
  return await isActiveStaffForCustomerInsert(userClient, em)
}
...
export async function POST(request: NextRequest) {
  const userSb = await getSupabaseForApiRoute(request)
  if (userSb instanceof NextResponse) {
  ...
  if (!(await callerMayInsertCustomers(userSb, user.email))) {
