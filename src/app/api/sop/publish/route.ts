...
import { sendSopPublishedStaffPush } from '@/lib/sendStaffSopPush'
import {
  ...
  try {
    ...
    if (!structure || !isPublishableSopDocument(structure)) {
      return NextResponse.json(
        { error: 'SOP 구조(섹션·카테고리·내용)가 비어 있거나 올바르지 않습니다' },
        { status: 400 }
      )
    }
    ...
    if (teamErr) {
      console.error('SOP publish team lookup:', teamErr)
      return NextResponse.json({ error: '권한 확인에 실패했습니다' }, { status: 500 })
    }

    if (!canManageCompanySop(user.email, teamRow)) {
      return NextResponse.json({ error: 'SOP을 게시할 권한이 없습니다' }, { status: 403 })
    }
    ...
    if (insErr || !inserted) {
      console.error('SOP insert:', insErr)
      return NextResponse.json({ error: insErr?.message || '저장에 실패했습니다' }, { status: 500 })
    }

    const signPath = `/${locale}/sop/sign?version=${inserted.id}`
    const admin = getSupabaseAdmin()
    const pushResult = await sendSopPublishedStaffPush(admin, {
