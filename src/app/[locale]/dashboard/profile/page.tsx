...
export default function ProfileRedirect() {
  ...
  useEffect(() => {
    const redirectToCustomerProfile = async () => {
      ...
      try {
        ...
        if (currentUser.id) {
          ...
          if (linkData) {
            ...
            console.log('ProfileRedirect: user_customer_links를 통해 고객 ID 발견:', customerId)
          }
        }
        ...
        if (!customerId) {
          ...
          if (customerData && typeof customerData === 'object' && 'id' in customerData) {
            ...
            console.log('ProfileRedirect: 이메일로 고객 ID 발견:', customerId)
          }
        }

        if (customerId) {
          // 고객별 프로필 페이지로 리다이렉트
          router.push(`/${locale}/dashboard/profile/${customerId}`)
        } else {
