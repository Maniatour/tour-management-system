              ...
              {sortedMembers.map((member) => {
                ...
                return (
                  <TeamMemberCard
                    ...
                    paymentMethods={paymentMethodsByUserEmail[member.email.toLowerCase()] ?? []}
                    documentCount={documentCount}
                    onOpenFullEdit={() => {
                      setEditingMember(member)
                      setShowForm(true)
                    }}
                    onQuickEdit={(field) => setQuickEdit({ email: member.email, field })}
                    onToggleActive={() => {
                      void handleToggleActive(member.email, member.is_active ?? true)
                    }}
                  />
                )
              })}
