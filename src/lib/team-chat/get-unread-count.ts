...
/** 사용자의 모든 팀 채팅방 안읽은 메시지 수 (RLS 적용 JWT 클라이언트) */
export async function getTeamChatUnreadCount(request: NextRequest) {
  try {
    ...
    const { data: unreadData, error: unreadError } = await supabaseUser
      ...
      .select('room_id, id, sender_email')
      .in('room_id', roomIds)
    ...
    const roomCounts = unreadCounts.reduce(
      (acc, room) => {
