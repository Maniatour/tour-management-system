...
/** 가이드 스케줄 배정 — 수신자 이메일의 staff_push_subscriptions로 web push */
export async function sendGuideScheduleAssignmentPush(
  admin: SupabaseClient,
  params: {
    tourId: string
    targetEmail: string
  ...
  let sent = 0
  let failed = 0
  const openUrl = params.confirmUrl.startsWith('http')
    ? params.confirmUrl
    : `${baseUrl}${params.confirmUrl.startsWith('/') ? '' : '/'}${params.confirmUrl}`
  ...
  await Promise.all(
    matched.map(async (subscription) => {
      ...
      const payload = JSON.stringify({
        ...
        icon: iconUrl,
        badge: iconUrl,
        tag: `guide-assignment-${params.tourId}-${email}`,
        data: {
          url: openUrl,
          tourId: params.tourId,
          type: 'guide_schedule_assignment',
        },
      })
      try {
        await webpush.sendNotification(
