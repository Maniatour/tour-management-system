...
export async function fetchTicketBookingStatementReconDisplayByBookingId(
  ...
  type MatchRow = { source_id: string; statement_line_id: string }
  ...
  for (let i = 0; i < ids.length; i += MATCH_CHUNK) {
    ...
    for (const row of (data || []) as MatchRow[]) {
      ...
      if (sid && lid) matches.push({ source_id: sid, statement_line_id: lid })
    }
  }
  ...
  for (let i = 0; i < ids.length; i += MATCH_CHUNK) {
    ...
    for (const row of (data || []) as { id: string; statement_line_id: string | null }[]) {
      ...
      if (row.id && lid && !matches.some((m) => m.source_id === row.id && m.statement_line_id === lid)) {
        matches.push({ source_id: row.id, statement_line_id: lid })
      }
  ...
  const toDisplay = (lineId: string): TicketBookingStatementReconDisplay | null => {
  ...
  for (const m of matches) {
    const disp = toDisplay(m.statement_line_id)
    if (!disp) continue
    const list = out.get(m.source_id) ?? []
    if (!list.some((x) => x.posted_date === disp.posted_date && x.description === disp.description)) {
      list.push(disp)
    }
