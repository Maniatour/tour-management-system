...
function WhatsAppIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden style={{ display: 'block' }}>
...
function LineIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" aria-hidden style={{ display: 'block' }}>
...
export function renderCustomerCommunicationChannelIcon(
  ...
  // SVG는 기본 inline baseline이라 옆 img/아이콘보다 위로 떠 보일 수 있음 → block으로 맞춤
  const merged = `block shrink-0 ${className} ${toneClass}`.trim()
