-- 날짜 노트: 가이드 스케줄 영역 노란색 강조 여부 (기본값 true = 기존 동작 유지)
ALTER TABLE public.date_notes
  ADD COLUMN IF NOT EXISTS highlight_guide_schedule boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.date_notes.highlight_guide_schedule IS
  'true면 스케줄뷰 가이드 그리드 해당 날짜 셀을 노란색으로 강조';

-- get_schedule_display RPC: date_notes에 highlight_guide_schedule 포함
