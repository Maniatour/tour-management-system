-- 자동 투어 생성 함수
-- Mania Tour 또는 Mania Service 예약이 생성될 때 자동으로 투어를 생성하거나 기존 투어에 추가하는 함수

CREATE OR REPLACE FUNCTION auto_create_or_update_tour()
RETURNS TRIGGER AS $$
DECLARE
    product_sub_category TEXT;
    existing_tour_id UUID;
    new_tour_id UUID;
BEGIN
...
-- reservations 테이블에 트리거 생성
CREATE TRIGGER auto_create_tour_trigger
    AFTER INSERT ON reservations
    FOR EACH ROW
    EXECUTE FUNCTION auto_create_or_update_tour();
