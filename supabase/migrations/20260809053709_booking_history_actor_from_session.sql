-- booking_history: attribute updates to the session actor, not original submitted_by.
-- apply_ticket_booking_action: set app.current_user_email from p_actor / JWT before UPDATE.
...
DECLARE
  actor text;
BEGIN
  actor := NULLIF(TRIM(COALESCE(current_setting('app.current_user_email', true), '')), '');
  ...
  -- INSERT only: fall back to original submitter
  IF actor IS NULL AND TG_OP = 'INSERT' THEN
    actor := NULLIF(TRIM(COALESCE(NEW.submitted_by, '')), '');
  END IF;
...
CREATE OR REPLACE FUNCTION public.apply_ticket_booking_action(p_booking_id text, p_action text, p_payload jsonb DEFAULT '{}'::jsonb, p_actor text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
DECLARE
  ...
  v_set_vs text;
  v_rn text;
  v_qty_differs boolean;
  v_time_differs boolean;
  v_apply_qty boolean;
  v_apply_time boolean;
  v_still_pending boolean;
  v_new_ea integer;
  v_confirm_handled boolean;
BEGIN
  -- Attribute booking_history / audits to the acting user (not original submitted_by)
  PERFORM set_config(
    'app.current_user_email',
    COALESCE(
      NULLIF(TRIM(p_actor), ''),
      NULLIF(TRIM(auth.jwt() ->> 'email'), ''),
      NULLIF(TRIM(COALESCE(current_setting('app.current_user_email', true), '')), '')
    ),
    true
  );
  ...
  ELSIF p_action = 'workflow_vendor_confirm_change' THEN
    ...
    v_qty_differs := r_old.pending_ea IS DISTINCT FROM r_old.ea;
    v_time_differs :=
      nullif(trim(coalesce(r_old.pending_time::text, '')), '') IS NOT NULL
      AND trim(coalesce(r_old.pending_time::text, '')) IS DISTINCT FROM trim(coalesce(r_old.time::text, ''));

    v_apply_qty := CASE
      WHEN p_payload ? 'apply_qty' THEN COALESCE((p_payload->>'apply_qty')::boolean, false)
      ELSE true
    END;
    v_apply_time := CASE
      WHEN p_payload ? 'apply_time' THEN COALESCE((p_payload->>'apply_time')::boolean, false)
      ELSE true
    END;

    IF v_apply_qty AND NOT v_qty_differs THEN
      v_apply_qty := false;
    END IF;
    IF v_apply_time AND NOT v_time_differs THEN
      v_apply_time := false;
    END IF;

    v_confirm_handled := false;
    IF NOT v_apply_qty AND NOT v_apply_time THEN
      IF r_old.pending_ea = 0 THEN
        v_apply_qty := true;
      ELSIF v_qty_differs OR v_time_differs THEN
        v_apply_qty := v_qty_differs;
        v_apply_time := v_time_differs;
      ELSE
        UPDATE public.ticket_bookings SET
        ...
        WHERE id = p_booking_id
        RETURNING * INTO r_new;
        v_confirm_handled := true;
      END IF;
    END IF;

    IF NOT v_confirm_handled AND r_old.pending_ea = 0 AND v_apply_qty THEN
      UPDATE public.ticket_bookings SET
      ...
      WHERE id = p_booking_id
      RETURNING * INTO r_new;
    ELSIF NOT v_confirm_handled THEN
      v_new_ea := CASE
        WHEN v_apply_qty AND v_qty_differs THEN r_old.pending_ea
        ELSE r_old.ea
      END;

      v_still_pending :=
        (v_qty_differs AND NOT (v_apply_qty AND v_qty_differs))
        OR (v_time_differs AND NOT (v_apply_time AND v_time_differs));

      UPDATE public.ticket_bookings SET
        ea = v_new_ea,
        time = CASE
          WHEN v_apply_time AND v_time_differs
          THEN r_old.pending_time::time
          ELSE r_old.time
        END,
        expense = CASE
          WHEN NOT (v_apply_qty AND v_qty_differs) THEN r_old.expense
          WHEN v_new_ea IS NOT DISTINCT FROM r_old.ea THEN r_old.expense
          WHEN r_old.unit_price IS NOT NULL AND (r_old.unit_price::numeric) > 0 THEN
            round((r_old.unit_price::numeric * v_new_ea::numeric)::numeric, 2)
          WHEN r_old.ea IS NOT NULL AND r_old.ea > 0
               AND r_old.expense IS NOT NULL AND (r_old.expense::numeric) > 0 THEN
            round(((r_old.expense::numeric / r_old.ea::numeric) * v_new_ea::numeric)::numeric, 2)
          ELSE r_old.expense
        END,
        booking_status = CASE
          WHEN v_still_pending THEN r_old.booking_status
          ELSE COALESCE(r_old.booking_status_before_change, 'confirmed'::text)
        END,
        vendor_status = CASE
          WHEN v_still_pending THEN 'pending'::text
          ELSE 'confirmed'::text
        END,
        change_status = CASE
          WHEN v_still_pending THEN 'requested'::text
          ELSE 'none'::text
        END,
        pending_ea = CASE
          WHEN v_qty_differs AND NOT (v_apply_qty AND v_qty_differs) THEN r_old.pending_ea
          ELSE NULL
        END,
        pending_time = CASE
          WHEN v_time_differs AND NOT (v_apply_time AND v_time_differs) THEN r_old.pending_time
          ELSE NULL
        END,
        booking_status_before_change = CASE
          WHEN v_still_pending THEN r_old.booking_status_before_change
          ELSE NULL
        END,
        updated_at = now()
      WHERE id = p_booking_id
  ...
  ELSIF p_action = 'workflow_complete_payment' THEN
    UPDATE public.ticket_bookings SET
      ...
      END,
      payment_method = CASE
        WHEN (p_payload ? 'payment_method') AND nullif(trim(p_payload->>'payment_method'), '') IS NOT NULL
        THEN trim(p_payload->>'payment_method')
        ELSE payment_method
      END,
      updated_at = now()
    WHERE id = p_booking_id
