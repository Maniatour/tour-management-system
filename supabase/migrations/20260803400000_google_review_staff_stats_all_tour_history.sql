-- 투어 진행·배정 인원·예약 건수: 최초 리뷰일 필터 없이 전체 tours 테이블 기준
...
comment on function public.admin_google_review_staff_stats is
  'Guide/assistant review stats (all platforms). Tour departure/guest/booking totals use full tours table history (not limited to first review date).';
