-- 월별 직원 리뷰 통계: 등록일(고객이 리뷰를 남긴 날) / 투어일 기준 분리

drop function if exists public.admin_google_review_staff_stats_monthly(uuid, int);
drop function if exists public.admin_google_review_staff_stats_monthly(uuid, int, text);
drop function if exists public.admin_google_review_staff_stat_reviews(uuid, text, int, int, int);
drop function if exists public.admin_google_review_staff_stat_reviews(uuid, text, int, int, int, text);

create or replace function public.admin_google_review_staff_stats_monthly(
  p_operator_id uuid,
  p_year int,
  p_month_by text default 'tour_date'
)
returns table (
  staff_email text,
  staff_name text,
...
as $$
  with staff_reviews as (
    select distinct
      ...
      coalesce(
        ...
        (gr.imported_at at time zone 'America/Los_Angeles')::date
      ) as review_date,
      t_linked.tour_date
  ...
  staff_reviews_anchored as (
    select
      sr.staff_email,
      sr.review_id,
      sr.rating,
      case
        when lower(coalesce(p_month_by, 'tour_date')) = 'review_date' then sr.review_date
        else sr.tour_date
      end as anchor_date
  ...
  review_by_month as (
    select
      sra.staff_email,
      extract(month from sra.anchor_date)::int as month_num,
      count(distinct sra.review_id)::bigint as review_count,
      round(avg(sra.rating)::numeric, 2) as avg_rating,
      count(distinct sra.review_id) filter (where sra.rating = 5)::bigint as five_star_count,
      count(distinct sra.review_id) filter (where sra.rating = 4)::bigint as four_star_count,
      count(distinct sra.review_id) filter (where sra.rating = 3)::bigint as three_star_count,
      count(distinct sra.review_id) filter (where sra.rating = 2)::bigint as two_star_count,
      count(distinct sra.review_id) filter (where sra.rating = 1)::bigint as one_star_count
    from staff_reviews_anchored sra
    where sra.anchor_date is not null
      and extract(year from sra.anchor_date)::int = p_year
    group by sra.staff_email, extract(month from sra.anchor_date)::int
  ),
...
as $$
  ...
  where lower(srl.staff_email) = lower(btrim(p_staff_email))
    ...
    and (
      ...
      or (
        case
          when lower(coalesce(p_month_by, 'tour_date')) = 'review_date' then srl.review_date
          else srl.tour_date
        end is not null
...
grant execute on function public.admin_google_review_staff_stats_monthly(uuid, int, text)
  to service_role, authenticated;

grant execute on function public.admin_google_review_staff_stat_reviews(uuid, text, int, int, int, text)
  to service_role, authenticated;

comment on function public.admin_google_review_staff_stats_monthly is
  'Monthly staff review stats. p_month_by = review_date (when the guest posted) or tour_date (tour departure). Guest/group counts stay on tour_date.';

comment on function public.admin_google_review_staff_stat_reviews is
  'List reviews for a staff member at a given star rating. Optional year/month filter uses p_month_by (review_date or tour_date).';
