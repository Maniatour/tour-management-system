-- 정규화 UI: 그룹 내 표준 결제내용(standard_paid_for) 미입력 건수(필터용)
begin;
...
as $$
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'missing_standard_count', q.missing_standard_count,
  ...
  from (
    select
      ce.paid_for,
      count(*)::int as cnt,
      ce.paid_for_label_id,
      count(*) filter (
        where ce.standard_paid_for is null
          or btrim(ce.standard_paid_for::text) = ''
      )::int as missing_standard_count
...
comment on function public.company_expense_paid_for_normalization_stats() is
  '결제 내용별 건수·라벨·표준 미입력 건수·대표 표준값(정규화 UI용). sample_* 는 그룹 내 한 행 기준.';
