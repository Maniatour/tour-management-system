-- 예약별 투어 신청 언어 (고객 모국어 customers.language 와 분리)
ALTER TABLE public.reservations
  ADD COLUMN IF NOT EXISTS tour_language text;

COMMENT ON COLUMN public.reservations.tour_language IS
  '투어 신청 언어 (ko/en/ja). 고객 모국어(customers.language)와 별개. 2026-09-11 이전 일본인 예약은 en.';

-- 기존 예약: 한국인만 한국어, 그 외(일본인 포함)는 영어
UPDATE public.reservations r
SET tour_language = CASE
  WHEN lower(trim(coalesce(c.language, ''))) IN (
    'ko', 'kr', 'kor', 'korean', '한국어', '한글'
  )
    OR lower(trim(coalesce(c.language, ''))) LIKE 'ko%'
    OR lower(trim(coalesce(c.language, ''))) LIKE 'kr%'
    OR coalesce(c.language, '') LIKE '%한국%'
  THEN 'ko'
  ELSE 'en'
END
FROM public.customers c
WHERE r.customer_id = c.id
  AND r.tour_language IS NULL;

UPDATE public.reservations
SET tour_language = 'en'
WHERE tour_language IS NULL;

-- 2026-09-11 이후 실제로 일본어 투어로 신청된 건만 일본어로 교정 (GyG Tour language 등)
UPDATE public.reservations r
SET tour_language = 'ja'
FROM public.reservation_imports i
WHERE i.reservation_id = r.id
  AND r.tour_language = 'en'
  AND r.tour_date >= DATE '2026-09-11'
  AND (
    coalesce(i.extracted_data->>'tour_language', '') ~* '^(ja|jp|japanese)'
    OR coalesce(i.raw_body_text, '') ~* 'tour\s*language\s*:?\s*japan'
    OR coalesce(i.raw_body_html, '') ~* 'tour\s*language\s*:?\s*japan'
  );

ALTER TABLE public.reservations
  DROP CONSTRAINT IF EXISTS reservations_tour_language_check;

ALTER TABLE public.reservations
  ADD CONSTRAINT reservations_tour_language_check
  CHECK (tour_language IS NULL OR tour_language IN ('ko', 'en', 'ja'));
...
COMMENT ON FUNCTION public.get_schedule_display(uuid, date, date, date, date) IS
  'Schedule view payload; reservations include tour_language for KO/EN/JA grouping.';
