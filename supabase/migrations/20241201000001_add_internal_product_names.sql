-- Add internal product name fields for admin use
-- These are shorter, simpler names used internally vs customer-facing names

-- Add internal name fields
ALTER TABLE products ADD COLUMN IF NOT EXISTS internal_name_ko VARCHAR(100);
ALTER TABLE products ADD COLUMN IF NOT EXISTS internal_name_en VARCHAR(100);

-- Add customer-facing name fields (rename existing fields for clarity)
ALTER TABLE products ADD COLUMN IF NOT EXISTS customer_name_ko VARCHAR(255);
ALTER TABLE products ADD COLUMN IF NOT EXISTS customer_name_en VARCHAR(255);
...
-- This is a temporary mapping - should be updated manually for each product
UPDATE products 
SET 
  internal_name_ko = CASE 
    ...
    WHEN name_ko IS NOT NULL AND LENGTH(name_ko) > 0 THEN LEFT(name_ko, 10) -- Take first 10 characters as fallback
    ELSE '상품' -- Default fallback for NULL or empty names
  END,
  internal_name_en = CASE 
    ...
    WHEN name_en IS NOT NULL AND LENGTH(name_en) > 0 THEN LEFT(name_en, 15) -- Take first 15 characters as fallback
    ELSE 'Product' -- Default fallback for NULL or empty names
  END
WHERE internal_name_ko IS NULL;
