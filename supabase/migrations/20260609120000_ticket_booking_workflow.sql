-- 입장권 부킹: 워크플로우용 컬럼·환불 라인·RPC 액션
-- pending_* = 변경 요청 중 벤더 미응답 시 표시용 (확정 시 ea/time 반영)

ALTER TABLE public.ticket_bookings
  ADD COLUMN IF NOT EXISTS pending_ea integer,
  ADD COLUMN IF NOT EXISTS pending_time text,
  ADD COLUMN IF NOT EXISTS booking_status_before_change text;

COMMENT ON COLUMN public.ticket_bookings.pending_ea IS '변경 요청된 수량 (벤더 응답 대기)';
COMMENT ON COLUMN public.ticket_bookings.pending_time IS '변경 요청된 시간 문자열 (HH:MM 등)';
COMMENT ON COLUMN public.ticket_bookings.booking_status_before_change IS '변경 요청 전 예약 축 상태 스냅샷';

-- ---------------------------------------------------------------------------
-- RN/부킹 단위 환불 서브 건 (메인 확정 행은 유지)
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS public.ticket_booking_refund_lines (
  id text PRIMARY KEY DEFAULT gen_random_uuid()::text,
  anchor_booking_id text NOT NULL REFERENCES public.ticket_bookings(id) ON DELETE CASCADE,
  status text NOT NULL DEFAULT 'requested' CHECK (
    status = ANY (ARRAY['requested','rejected','refunded','credit_received']::text[])
  ),
  amount numeric(12, 2),
  ea integer,
  note text,
...
CREATE INDEX IF NOT EXISTS idx_ticket_booking_refund_lines_anchor
  ON public.ticket_booking_refund_lines (anchor_booking_id, created_at DESC);

COMMENT ON TABLE public.ticket_booking_refund_lines IS '확정 부킹에 연결된 환불·크레딧 서브 행';
...
CREATE POLICY "Enable all access for ticket_booking_refund_lines"
  ON public.ticket_booking_refund_lines FOR ALL USING (true) WITH CHECK (true);

-- ---------------------------------------------------------------------------
-- 레거시 status 파생: tentative + 변경 요청
-- ---------------------------------------------------------------------------
...
AS $$
  SELECT CASE
    ...
    WHEN cs = 'requested' AND bs IN ('confirmed', 'tentative') THEN 'guest_change_requested'
...
-- ---------------------------------------------------------------------------
-- apply_ticket_booking_action: 워크플로우 액션 (set_axes 직후 ~ report_issue 직전에 삽입)
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.apply_ticket_booking_action(text, text, jsonb, text) IS
  '단일 진입점: 액션·set_axes·워크플로우(벤더 응답·변경·결제 완료).';
