-- get_schedule_display: tours JSON에 assignment_status 포함 (스케줄뷰 가이드 그리드 배정 상태 아이콘용)
...
COMMENT ON FUNCTION public.get_schedule_display(uuid, date, date, date, date) IS
  'Schedule view payload; tours include assignment_status for guide grid badges.';
