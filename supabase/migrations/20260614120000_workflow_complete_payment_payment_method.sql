-- workflow_complete_payment: optional payment_method in p_payload (trim, non-empty updates column)
  ...
  ELSIF p_action = 'workflow_vendor_confirm_change' THEN
    ...
    ELSE
      UPDATE public.ticket_bookings SET
        ...
        END,
        expense = CASE
          WHEN r_old.pending_ea IS NOT DISTINCT FROM r_old.ea THEN r_old.expense
          WHEN r_old.unit_price IS NOT NULL AND (r_old.unit_price::numeric) > 0 THEN
            round((r_old.unit_price::numeric * r_old.pending_ea::numeric)::numeric, 2)
          WHEN r_old.ea IS NOT NULL AND r_old.ea > 0
               AND r_old.expense IS NOT NULL AND (r_old.expense::numeric) > 0 THEN
            round(((r_old.expense::numeric / r_old.ea::numeric) * r_old.pending_ea::numeric)::numeric, 2)
          ELSE r_old.expense
        END,
