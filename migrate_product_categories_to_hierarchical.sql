-- 기존 product_sub_categories 테이블을 카테고리 종속 구조로 마이그레이션
-- 기존 테이블이 있다면 수정하고, 없다면 새로 생성

-- 1. 기존 테이블이 있는지 확인하고 수정
DO $$
BEGIN
...
-- RLS 설정 (테이블이 새로 생성된 경우에만)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'product_sub_categories') THEN
...
-- updated_at 자동 갱신 트리거 (테이블이 새로 생성된 경우에만)
DO $$
BEGIN
...
-- 기존 products 테이블의 데이터를 새 구조로 마이그레이션
-- 카테고리 데이터 먼저 삽입
