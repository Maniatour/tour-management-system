...
SELECT 
    ...
    true as is_active,
    'admin@example.com' as created_by
FROM tours t
...
-- 9. 결과 확인
SELECT '=== 설정 완료된 데이터 현황 ===' as status;
...
UNION ALL
SELECT '채팅방' as table_name, COUNT(*) as count FROM chat_rooms;

-- 10. 오늘 이후 투어 및 채팅방 현황 확인
SELECT 
    '오늘 이후 투어 및 채팅방 현황' as status,
    COUNT(t.id) as total_tours,
    COUNT(CASE WHEN t.tour_status = 'Recruiting' THEN 1 END) as recruiting_tours,
    COUNT(CASE WHEN t.tour_status = 'Confirmed' THEN 1 END) as confirmed_tours,
    COUNT(cr.id) as chat_rooms_created
FROM tours t
LEFT JOIN chat_rooms cr ON t.id = cr.tour_id
WHERE t.tour_date >= CURRENT_DATE;

-- 11. 투어별 채팅방 상세 현황 (오늘 이후 투어만)
SELECT 
    t.id as tour_id,
    t.tour_date,
