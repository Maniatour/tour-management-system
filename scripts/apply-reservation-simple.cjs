...
const ifaceFrom = `  /** reservations.tour_id가 비어 있을 때 tours.reservation_ids 기준 투어 ID */
  linkedTourId?: string | null
}

export const ReservationCardItem`
const ifaceTo = `  /** reservations.tour_id가 비어 있을 때 tours.reservation_ids 기준 투어 ID */
  linkedTourId?: string | null
  /** 카드뷰 밀도: 간단(4줄) / 상세(기본) */
...
function reservationStatusLucideIcon(statusLower: string) {
  ...
  if (statusLower === 'completed') return <CircleCheck className={\`\${className} text-blue-800\`} aria-hidden />
...
export const ReservationCardItem`
if (!s.includes(ifaceFrom)) throw new Error('iface block missing')
s = s.replace(ifaceFrom, ifaceTo)

const destFrom = `  choicesCacheRef,
  linkedTourId = null
}: ReservationCardItemProps) {`
const destTo = `  choicesCacheRef,
  linkedTourId = null,
  cardLayout = 'standard'
}: ReservationCardItemProps) {`
if (!s.includes(destFrom)) throw new Error('destructure missing')
s = s.replace(destFrom, destTo)

const needle = `      setStatusUpdating(false)
    }
  }
...
const simpleBlock = `      setStatusUpdating(false)
    }
  }
...
if (!s.includes(needle)) throw new Error('needle missing for simple insert')
s = s.replace(needle, simpleBlock)

const memoFrom = `    prevProps.linkedTourId === nextProps.linkedTourId &&
    prevProps.tourInfoMap === nextProps.tourInfoMap &&`
const memoTo = `    prevProps.linkedTourId === nextProps.linkedTourId &&
    prevProps.cardLayout === nextProps.cardLayout &&
    prevProps.tourInfoMap === nextProps.tourInfoMap &&`
if (!s.includes(memoFrom)) throw new Error('memo block missing')
s = s.replace(memoFrom, memoTo)

fs.writeFileSync(p, s)
console.log('ReservationCardItem patched')
