...
-- 2. 기존 JSONB choices를 새로운 구조로 변환하는 함수 (변수명 충돌 완전 방지)
...
BEGIN
  ...
  LOOP
    ...
    RETURN NEXT current_product_id, choices_count, options_count;
  END LOOP;
...
BEGIN
  ...
  LOOP
    ...
    RETURN NEXT reservation_record.id, choices_count;
  END LOOP;
