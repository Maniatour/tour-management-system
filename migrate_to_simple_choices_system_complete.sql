-- 기존 JSONB choices 데이터를 새로운 간결한 구조로 마이그레이션 (최종 수정본)
...
-- 2. 기존 JSONB choices를 새로운 구조로 변환하는 함수 (RETURNS SETOF 사용)
...
BEGIN
  ...
  LOOP
    ...
    IF product_record.choices ? 'required' AND jsonb_typeof(product_record.choices->'required') = 'array' THEN
      ...
      LOOP
        ...
        IF choice_item ? 'options' AND jsonb_typeof(choice_item->'options') = 'array' THEN
          ...
          LOOP
            option_key_name := COALESCE(choice_option->>'id', 'default_option');
    ...
    IF product_record.choices ? 'optional' AND jsonb_typeof(product_record.choices->'optional') = 'array' THEN
      ...
      LOOP
        choice_group_name := COALESCE(choice_item->>'id', 'optional_choice');
    ...
    -- 결과 반환
    SELECT current_product_id, choices_count, options_count INTO result;
    RETURN NEXT result;
  END LOOP;
...
BEGIN
  ...
  LOOP
    ...
    -- 결과 반환
    SELECT reservation_record.id, choices_count INTO result;
    RETURN NEXT result;
  END LOOP;
...
BEGIN
  ...
  -- 상품 choices 마이그레이션
  FOR result IN SELECT * FROM migrate_existing_choices_to_simple() AS (product_id TEXT, migrated_choices INTEGER, migrated_options INTEGER) LOOP
  ...
  -- 예약 choices 마이그레이션
  FOR result IN SELECT * FROM migrate_existing_reservation_choices_to_simple() AS (reservation_id TEXT, migrated_choices INTEGER) LOOP
